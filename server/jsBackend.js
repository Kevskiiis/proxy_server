...
export function startJsBackend({ port }) {
  ...
  server.on('error', (error) => {
    console.error(`Failed to start the JS backend on port ${port}. ${error.message}`)
    process.exitCode = 1
  })
  ...
  async function start() {
    ...
    server.listen(port, () => {
      console.log(`CS Guardian JS API listening on http://localhost:${port}`)
    })
  }
